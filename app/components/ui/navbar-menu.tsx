					...
					{active === item && (
						<div className="absolute top-[calc(100%_+_1.5rem)] left-1/2 transform -translate-x-1/2 pt-4">
							<motion.div
								...
								className="bg-white dark:bg-black-100 backdrop-blur-sm rounded-2xl overflow-hidden border border-black/[0.2] dark:border-white/[0.2] shadow-xl"
							>
								<motion.div
...
export const Menu = ({ setActive, children }: { setActive: (item: string | null) => void; children: React.ReactNode }) => {
	return (
		<nav
			...
			className="relative rounded-full border border-transparent dark:bg-black-100 dark:border-white/[0.2] bg-white shadow-input flex justify-center space-x-4 px-8 py-6 "
		>
			{children}
		</nav>
	);
